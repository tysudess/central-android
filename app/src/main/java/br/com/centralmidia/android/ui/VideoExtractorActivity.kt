...
class VideoExtractorActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        downloadSection = downloadSection(); historySection = historySection(); settingsSection = settingsSection()
        root.addView(downloadSection, MobileUi.match(dp(10))); root.addView(historySection, MobileUi.match(dp(10))); root.addView(settingsSection, MobileUi.match(dp(10)))
        updateTabs()
    ...
    private fun updateTabs(){tabButtons.forEach{(tab,button)->val selected=tab==selectedTab;button.backgroundTintList=ColorStateList.valueOf(if(selected)MobileUi.BLUE else Color.WHITE);button.setTextColor(if(selected)Color.WHITE else MobileUi.NAVY);button.strokeColor=ColorStateList.valueOf(if(selected)MobileUi.BLUE else MobileUi.BORDER);button.strokeWidth=dp(1)};if(::downloadSection.isInitialized)downloadSection.visibility=if(selectedTab==Tab.DOWNLOAD)View.VISIBLE else View.GONE;if(::historySection.isInitialized)historySection.visibility=if(selectedTab==Tab.HISTORY)View.VISIBLE else View.GONE;if(::settingsSection.isInitialized)settingsSection.visibility=if(selectedTab==Tab.SETTINGS)View.VISIBLE else View.GONE;if(selectedTab==Tab.HISTORY&&::historyContainer.isInitialized)refreshHistory()}
